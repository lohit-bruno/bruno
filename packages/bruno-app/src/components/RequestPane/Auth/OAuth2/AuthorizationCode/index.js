...
const OAuth2AuthorizationCode = ({ save, item = {}, request, handleRun, updateAuth, collection, folder }) => {
  ...
  const preferenceCallbackUrl = preferences?.request?.oauth2?.callbackUrl || 'https://oauth2.usebruno.com';
    ...
    <StyledWrapper className="mt-2 flex w-full gap-4 flex-col">
      ...
      <div className="flex items-center gap-4 w-full" key="input-callbackUrl">
        ...
        <div className="flex flex-col gap-1 w-full">
          <div className={`single-line-editor-wrapper flex-1 flex items-center ${useSystemBrowser ? 'opacity-70' : ''}`}>
            <SingleLineEditor
              value={useSystemBrowser ? preferenceCallbackUrl : (callbackUrl || '')}
              theme={storedTheme}
              onSave={handleSave}
