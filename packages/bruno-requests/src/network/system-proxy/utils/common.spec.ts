...
describe('normalizeProxyUrl', () => {
  it('should add http protocol when missing', () => {
    expect(normalizeProxyUrl('proxy.usebruno.com:8080', 'http')).toBe('http://proxy.usebruno.com:8080');
  });

  it('should not modify URL with existing protocol', () => {
    expect(normalizeProxyUrl('http://proxy.usebruno.com:8080', 'http')).toBe('http://proxy.usebruno.com:8080');
    expect(normalizeProxyUrl('https://proxy.usebruno.com:8443', 'http')).toBe('https://proxy.usebruno.com:8443');
  });
  ...
  it('should handle various protocols', () => {
    expect(normalizeProxyUrl('socks5://proxy.usebruno.com:1080', 'http')).toBe('socks5://proxy.usebruno.com:1080');
    expect(normalizeProxyUrl('socks4://proxy.usebruno.com:1080', 'http')).toBe('socks4://proxy.usebruno.com:1080');
  });

  it('should handle URLs without port', () => {
    expect(normalizeProxyUrl('proxy.usebruno.com', 'http')).toBe('http://proxy.usebruno.com');
  });
});
