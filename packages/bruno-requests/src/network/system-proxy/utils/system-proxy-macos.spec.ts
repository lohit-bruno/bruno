import { MacOSProxyDetector } from './macos';
...
describe('MacOSProxyDetector', () => {
  let detector: MacOSProxyDetector;
  let mockExec: jest.MockedFunction<any>;

  beforeEach(() => {
    detector = new MacOSProxyDetector();
  ...
  ExcludeSimpleHostnames : 1
      ...
      mockExec.mockResolvedValueOnce({ stdout: scutilOutput, stderr: '' });
  ...
  describe('Error Handling', () => {
    it('should throw error when scutil command fails', async () => {
      mockExec.mockRejectedValueOnce(new Error('scutil command not found'));
    ...
    it('should throw error for invalid scutil output', async () => {
      mockExec.mockResolvedValueOnce({ stdout: 'Invalid output format', stderr: '' });
