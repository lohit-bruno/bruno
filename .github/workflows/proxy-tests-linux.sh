#!/usr/bin/env bash
#
# Local equivalent of the GitHub Actions Linux proxy test pipeline.
# Run from the repo root: bash .github/workflows/proxy-tests-linux.sh
#
# Prerequisites: sudo apt-get install squid-openssl apache2-utils openssl
#
set -euo pipefail
...
cleanup() {
  ...
  # Stop squid if running via sudo
  sudo squid -k shutdown -f "$SQUID_WORK_DIR/squid.conf" 2>/dev/null || true
...
MISSING_PKGS=()
command -v squid >/dev/null  || MISSING_PKGS+=(squid-openssl)
command -v openssl >/dev/null || MISSING_PKGS+=(openssl)
command -v htpasswd >/dev/null || MISSING_PKGS+=(apache2-utils)
...
if [ ${#MISSING_PKGS[@]} -gt 0 ]; then
  ...
  sudo apt-get update
  sudo apt-get --no-install-recommends install -y "${MISSING_PKGS[@]}"
fi
...
openssl req -x509 -newkey rsa:2048 \
  ...
  -days 3650 -nodes \
  -subj "/CN=localhost" \
  -addext "subjectAltName=IP:127.0.0.1,DNS:localhost,DNS:host.docker.internal" 2>/dev/null
...
# ---------------------------------------------------------------------------
# 4. Configure and start Squid proxy (requires sudo on Linux)
# ---------------------------------------------------------------------------
echo "==> Configuring Squid..."

# htpasswd — write to temp dir
...
# Find the basic_ncsa_auth helper (standard locations on Debian/Ubuntu)
NCSA_AUTH=$(find /usr/lib/squid /usr/lib/squid3 /usr/libexec/squid /usr -name "basic_ncsa_auth" -type f 2>/dev/null | head -1 || true)
if [ -z "$NCSA_AUTH" ]; then
  echo "ERROR: basic_ncsa_auth not found (is squid installed?)" && exit 1
fi
echo "Found basic_ncsa_auth at: $NCSA_AUTH"

# Log dir — must be writable by the 'proxy' user (squid's cache_effective_user)
SQUID_LOG_DIR="$SQUID_WORK_DIR/log"
mkdir -p "$SQUID_LOG_DIR"
sudo chown -R proxy:proxy "$SQUID_WORK_DIR"

# Detect squid version for correct https_port syntax
SQUID_MAJOR=$(squid -v 2>&1 | grep -oP 'Version \K[0-9]+' | head -1 || echo "5")
...
if [ "$SQUID_MAJOR" -ge 6 ]; then
  ...
  HTTPS_PORT_LINE="https_port 8091 cert=$CERT_DIR/squid.pem"
fi
...
sed -e "s|https_port 8091 cert=@SQUID_CERT@|$HTTPS_PORT_LINE|g" \
    -e "s|@SQUID_CERT@|$CERT_DIR/squid.pem|g" \
...
echo "==> Initializing Squid swap directories..."
sudo squid -z --foreground -f "$SQUID_CONF" 2>/dev/null || true

echo "==> Starting Squid proxy (HTTP :8090, HTTPS :8091)..."
# Remove stale PID file if present
rm -f "$SQUID_WORK_DIR/squid.pid"

sudo squid -N -f "$SQUID_CONF" &
SQUID_PID=$!
PIDS+=($SQUID_PID)
...
# xvfb-run npm run test:e2e:proxy || ((CLI_FAILURES++))
